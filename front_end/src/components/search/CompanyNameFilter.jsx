...
const CompanyNameFilter = ({ onChange, value, initialValue }) => {
  // Prefer fully controlled `value`, else `initialValue`
  const propValue = value || initialValue;

  // Handle legacy string value or new object value
  const getInitialInclude = () => {
    if (!propValue) return [];
    if (typeof propValue === 'string') return [propValue];
    if (propValue.include) return propValue.include;
    return [];
  };

  const [roleFilter, setRoleFilter] = useState({
    include: getInitialInclude(),
  ...
  // Dropdown UI logic
  const [dropdownOpen, setDropdownOpen] = useState({ include: false });
  const dropdownRefs = {
    include: useRef(null)
  };
  ...
  const notifyChange = (nextFilter, includeLabelsOverride, excludeLabelsOverride) => {
    ...
    onChange?.({
      ...
      includeLabels,
      excludeLabels,
      // aliases to match other filters
  ...
  useEffect(() => {
    ...
    if (typeof propValue === 'string') {
        newInclude = [propValue];
    } else {
        newInclude = (propValue.include || []).map(String);
        newExclude = (propValue.exclude || []).map(String);
    }
  ...
  // Helper to handle CSV File Upload
  const handleFileUpload = (e, type) => {
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
       ...
       // processExtractedItems is now defined outside
    });
  ...
  const processExtractedItems = (items, type) => {
    ...
    items.forEach(item => {
      ...
      if (rawDomain && rawDomain.includes('.')) {
        ...
        cleanDomain = item.name ? item.name.trim() : (rawDomain ? rawDomain.trim() : '');
      }

      // Generate Name Part
      let namePart = item.name ? item.name.trim() : (cleanDomain.includes('.') ? cleanDomain.split('.')[0] : cleanDomain);
      if (!item.name && namePart) {
      ...
      newIds.push(id);
      newLabels[id] = item.name ? item.name.trim() : cleanDomain; // Display name or domain
    });

    // Filter out items that are already selected
    const uniqueNewIds = newIds.filter(id => !roleFilter[type].includes(id));
    
    if (uniqueNewIds.length === 0) return;

    const nextFilter = {
      ...roleFilter,
      [type]: [...roleFilter[type], ...uniqueNewIds],
    };

    const nextLabels = type === "include" ? { ...labelsMap.include, ...newLabels } : { ...labelsMap.exclude, ...newLabels };

    setRoleFilter(nextFilter);
    if (type === "include") {
        setLabelsMap(prev => ({ ...prev, include: nextLabels }));
        notifyChange(nextFilter, nextLabels, null);
    } else {
        setLabelsMap(prev => ({ ...prev, exclude: nextLabels }));
        notifyChange(nextFilter, null, nextLabels);
    }
  };
  ...
  const handleSelect = (type) => (rawItem) => {
    ...
    if (roleFilter[type].some((x) => String(x) === idStr)) return;
  ...
  const handleRemove = (type) => (idToRemove) => {
    ...
    const nextLabels = { ...(type === "include" ? labelsMap.include : labelsMap.exclude) };
    delete nextLabels[idStr];
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Include Section Only for Company Search */}
