...
const YearsInCurrentCompanyFilter = ({ onChange, value, initialValue }) => {
  // Prefer fully controlled `value`, else `initialValue`
  const propValue = value || initialValue;

  // Internal state: keep IDs as strings
  ...
  const handleRemove = (idToRemove) => {
    ...
    delete nextLabels[id]; // keep or delete; deleting keeps payload tidy
    ...
    <div className="flex flex-col gap-4">
      ...
      >
        <option value="" disabled>
          Choose years in current company
        </option>
      ...
      <CustomDropdown
        title="Choose years in current company"
...
export default YearsInCurrentCompanyFilter;
